...
GROUPS = {
  :iscsi => {
    :"#{PRJ}-iscsi-01" => {
      :disks => { :"#{PRJ}-iscsi-01-disk01" => '20GB' },
      :intnets => {
        :"#{PRJ}-iscsi01" => { :ip => '10.131.0.11' },
        :"#{PRJ}-iscsi02" => { :ip => '10.132.0.11' },
      },
      :networks => { :private_network => { :ip => '192.168.56.11' } },
    },
    :"#{PRJ}-iscsi-02" => {
      :disks => { :"#{PRJ}-iscsi-02-disk01" => '20GB' },
      :intnets => {
        :"#{PRJ}-iscsi01" => { :ip => '10.131.0.12' },
        :"#{PRJ}-iscsi02" => { :ip => '10.132.0.12' },
      },
  ...
  :backend => {
    :"#{PRJ}-backend-01" => {
      :memory => 2048,
      :intnets => {
        :"#{PRJ}-iscsi01" => { :ip => '10.131.0.21' },
        :"#{PRJ}-iscsi02" => { :ip => '10.132.0.21' },
      },
    ...
    :"#{PRJ}-backend-02" => {
      :memory => 2048,
      :intnets => {
        :"#{PRJ}-iscsi01" => { :ip => '10.131.0.22' },
        :"#{PRJ}-iscsi02" => { :ip => '10.132.0.22' },
      },
    ...
    :"#{PRJ}-backend-03" => {
      :memory => 2048,
      :intnets => {
        :"#{PRJ}-iscsi01" => { :ip => '10.131.0.23' },
        :"#{PRJ}-iscsi02" => { :ip => '10.132.0.23' },
      },
...
ANSIBLE_HOSTVARS = MACHINES.each_with_object({}) {
  |kv, obj| obj[kv[0]] = {
    ...
    'ip_address_iscsi1' => kv[1].fetch(:intnets, {}).key?(:"#{PRJ}-iscsi01") ?
      kv[1][:intnets][:"#{PRJ}-iscsi01"][:ip] : nil,
    'ip_address_iscsi2' => kv[1].fetch(:intnets, {}).key?(:"#{PRJ}-iscsi02") ?
      kv[1][:intnets][:"#{PRJ}-iscsi02"][:ip] : nil,
  }
}
